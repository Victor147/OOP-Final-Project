...
void SVGFile::save(std::ofstream& file) {
	file << "<svg>\n";
...
void SVGFile::setFile(std::string& _fileName, std::string& _filePath, std::ifstream* _file) {
	fileName = _fileName;
	filePath = _filePath;
	loadVector(_file);
}

void SVGFile::saveFile() {
...
void SVGFile::addFigure(Figure* f) {
	figures.push_back(f);
	std::cout << "Successfully created a " << f->getType() << "! (" << figures.size() << ")\n";
}

void SVGFile::translate(double horizontal, double vertical, size_t ind) {
	if (ind == -1)
	{
		for (Figure* f : figures)
		{
...
void SVGFile::loadVector(std::ifstream* file) {
